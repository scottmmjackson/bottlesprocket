...
fn main() {
    let matches = ClapCommand::new("bottlesprocket")
    ...
    let portname = std::ffi::CString::new(
        matches.get_one::<Vec<u8>>("serial").unwrap().to_owned()
    ).unwrap();
